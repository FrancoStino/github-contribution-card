...
import { ContributorFetcher, renderContributorStatsCard } from '@/cards/stats-card';
...
function createRateLimitedFetcher(): ContributorFetcher {
  return async (
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'github-contribution-card-action',
    };
    ...
    if (response.status === 403 || response.status === 429) {
      ...
      if (retryAfter) {
        ...
        core.info(
          `Secondary rate limit hit. retry-after: ${retryAfter}s. Waiting ${Math.ceil(waitTime / 1000)}s...`,
        );
      }
      // Check x-ratelimit-reset (primary rate limit)
        ...
        core.info(
          `Primary rate limit reached (${rateLimitRemaining}/${rateLimitLimit}). Waiting ${Math.ceil(waitTime / 1000)}s until reset...`,
        );
      }
    ...
    if (requestCount === 1) {
      core.info(
        `Rate limit info: ${rateLimitRemaining}/${rateLimitLimit} remaining (resets at ${rateLimitReset ? new Date(parseInt(rateLimitReset, 10) * 1000).toISOString() : 'N/A'})`,
      );
    ...
    if (rateLimitRemaining && parseInt(rateLimitRemaining, 10) <= 1 && rateLimitReset) {
      ...
      core.info(
        `Primary rate limit almost exhausted (${rateLimitRemaining}/${rateLimitLimit}). Waiting ${Math.ceil(waitTime / 1000)}s until reset...`,
      );
      await sleep(waitTime);
    }
...
async function run(): Promise<void> {
  try {
    ...
    const outputFile = core.getInput('output-file') || 'github-contribution-card.svg';
